...
type Destination = { name: string; phone: string; note?: string }
type RoutePlan = { id: RouteId; name: string; detail: string; rvParks: Destination[]; hotels: Destination[] }

const routes: RoutePlan[] = [
  {
    id: 'EAST', name: 'Route A — East', detail: 'Highway 1 toward Burnaby, Langley, Abbotsford and Chilliwack. Reassess before continuing beyond Hope.',
    rvParks: [
  ...
  {
    id: 'ALT_EAST', name: 'Route B — Alternate East', detail: 'Highway 7 toward Pitt Meadows, Maple Ridge and Mission, then reassess toward the Fraser Valley.',
    rvParks: [
  ...
  {
    id: 'SOUTH', name: 'Route C — South', detail: 'Highway 99 / 91 / 10 / 17 network as directed by the emergency. Confirm the RV height and route restrictions.',
    rvParks: [],
    hotels: [{ name: 'Accent Inns Vancouver Airport — Richmond', phone: '604-273-3311', note: 'Dog-friendly southbound fallback. Confirm RV parking before arriving.' }],
  },
  {
    id: 'NORTH', name: 'Route D — North', detail: 'Highway 99 toward Squamish and Whistler only when the hazard direction and road conditions make north safe.',
    rvParks: [
    ...
    hotels: [{ name: 'Executive Suites Hotel & Resort Squamish', phone: '604-815-0048', note: 'Dog-friendly Sea-to-Sky fallback.' }],
  },
]
  ...
  return (
    <div className="evac-page">
      <header className="evac-hero">
        <div><p className="evac-kicker">Reusable household plan</p><h1>Evacuation Plan</h1><p>This page is the standing plan. It is not tied to one wildfire season or one incident.</p></div>
      ...
      <main className="evac-main">
        <section className="evac-panel">
          ...
          <div className="route-grid">{routes.map((route) => <button className={`route-card route-choice ${selectedRoute === route.id ? 'selected' : ''}`} key={route.id} onClick={() => setSelectedRoute(route.id)}><h3>{route.name}</h3><p>{route.detail}</p></button>)}</div>
        ...
        {activeRoute && <section className="evac-panel route-destinations">
          <div className="evac-heading"><div><p className="evac-kicker">Selected corridor</p><h2>{activeRoute.name}</h2></div><button className="route-clear" onClick={() => setSelectedRoute(null)}>Clear route</button></div>
          <p className="selected-route-detail">{activeRoute.detail}</p>
          <h3>RV parks / campgrounds</h3>
          {activeRoute.rvParks.length ? <div className="destination-grid">{activeRoute.rvParks.map((destination) => <DestinationCard key={destination.name} destination={destination} />)}</div> : <div className="evac-note">No standing RV-park option has been assigned to this corridor yet. Do not assume a hotel parking lot can take the motorhome; call ahead.</div>}
          <h3>Dog-friendly hotel / motel fallbacks</h3>
          <div className="destination-grid">{activeRoute.hotels.map((destination) => <DestinationCard key={destination.name} destination={destination} />)}</div>
        ...
        <section className="evac-panel">
          ...
          <a className="evac-note" href="https://ess.gov.bc.ca/" target="_blank" rel="noreferrer"><strong>Open the official BC Emergency Support Services portal</strong><br />Create or access your Evacuee Registration and Assistance (ERA) profile and follow incident instructions.</a>
        ...
        <section className="evac-panel">
          ...
          <div className="form-grid">
            <Field label="Insurance company" value={fields.insurer} onChange={(v) => set('insurer', v)} /><Field label="Emergency claims number" value={fields.claims} onChange={(v) => set('claims', v)} /><Field label="Policy number" value={fields.policy} onChange={(v) => set('policy', v)} /><Field label="Veterinarian" value={fields.vet} onChange={(v) => set('vet', v)} /><Field label="Vet telephone" value={fields.vetPhone} onChange={(v) => set('vetPhone', v)} /><Field label="Emergency veterinary clinic" value={fields.emergencyVet} onChange={(v) => set('emergencyVet', v)} />
          </div>
        </section>
        ...
        <section className="evac-panel">
          ...
          <div className="form-grid">
            <Field label="Height to highest rooftop component" value={fields.rvHeight} onChange={(v) => set('rvHeight', v)} /><Field label="Overall length" value={fields.rvLength} onChange={(v) => set('rvLength', v)} /><Field label="Width including mirrors" value={fields.rvWidth} onChange={(v) => set('rvWidth', v)} /><Field label="Approx. loaded weight" value={fields.rvWeight} onChange={(v) => set('rvWeight', v)} /><Field label="Fuel type" value={fields.rvFuel} onChange={(v) => set('rvFuel', v)} /><Field label="Safe range on one tank" value={fields.rvRange} onChange={(v) => set('rvRange', v)} /><Field label="Licence plate" value={fields.rvPlate} onChange={(v) => set('rvPlate', v)} /><Field label="Propane tank / cylinders" value={fields.rvPropane} onChange={(v) => set('rvPropane', v)} />
          </div>
        ...
        <section className="evac-panel">
          ...
          <div className="form-grid"><Field label="Rally A — local" value={fields.rallyA} onChange={(v) => set('rallyA', v)} /><Field label="Rally B — east" value={fields.rallyB} onChange={(v) => set('rallyB', v)} /><Field label="Rally C — alternate" value={fields.rallyC} onChange={(v) => set('rallyC', v)} /></div>
        </section>

        <section className="evac-panel">
          <div className="evac-heading"><div><p className="evac-kicker">Two vehicles</p><h2>RV sets the pace</h2></div></div>
          <ul className="evac-list"><li>The RV is the pace-setting vehicle; the SUV is the more agile support vehicle.</li><li>For the initial urban portion, the SUV follows the RV rather than disappearing ahead.</li><li>Once established on the main corridor, the SUV may scout ahead only where legal and safe.</li><li>Keep paper directions in both vehicles. Cellular navigation is not the only copy of the route.</li><li>If communication fails, neither driver turns around. Both proceed to the agreed rally point.</li></ul>
        </section>

        <section className="evac-panel">
          <div className="evac-heading"><div><p className="evac-kicker">Tucker</p><h2>Travel information</h2></div></div>
          <div className="form-grid"><Field label="Microchip / tattoo number" value={fields.tuckerId} onChange={(v) => set('tuckerId', v)} /><Field label="Medication" value={fields.tuckerMeds} onChange={(v) => set('tuckerMeds', v)} /><Field label="Normal travelling position" value={fields.tuckerTravel} onChange={(v) => set('tuckerTravel', v)} /><Field label="Trusted alternate caregiver" value={fields.tuckerCaregiver} onChange={(v) => set('tuckerCaregiver', v)} /></div>
...
function Contact({ label, value }: { label: string; value: string }) { return <article className="contact-card"><span>{label}</span><strong>{value}</strong></article> }
function DestinationCard({ destination }: { destination: Destination }) { return <article className="contact-card destination-card"><span>{destination.name}</span><strong>{destination.phone}</strong>{destination.note && <small>{destination.note}</small>}</article> }
function Field({ label, value = '', onChange }: { label: string; value?: string; onChange: (value: string) => void }) { return <label className="evac-field"><span>{label}</span><input value={value} onChange={(e) => onChange(e.target.value)} /></label> }
